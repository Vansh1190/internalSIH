import { IonBackButton, IonButton, IonButtons, IonContent, IonHeader, IonIcon, IonMenuButton, IonPage, IonTitle, IonToolbar, useIonRouter } from '@ionic/react';
// import { useStoreActions } from 'easy-peasy';
import { arrowBack, logOutOutline } from 'ionicons/icons';
import { useHistory, useLocation } from 'react-router';
// import { RefresherEventDetail } from '@ionic/core';
import './PageLayout.css'

const TOOLBAR_TITLE: any = {
  '/':"Dashboard",
  "/dashboard": "Dashboard",
  "/page/Profile": "Profile",
  "/page/Announcements": "Announcements",
  "/page/Events": "Events",
  "/page/Winner": "Winners",
  "/page/Certificate": "Certificate",
  "/page/MyEnrollments": "MyEnrollments",
  "/page/Attendence": "Attendance",
  "/dashboard/dev-team": "Dev Team",

  '/page/Analytics': "Analytics",
  "/admin": "Admin Dashboard",
  "/page/EnrolledUsers  ": "Enrolled Users",
  "/admin/mark-attendance": "Mark Attendance",
  "/admin/view-attendance": "View Attendance",
  "/page/UpdateResult": "Mark Result",
  "/admin/view-result": "View Result",
  // "/admin/promote-event": "Promote Event",
  "/page/Export": "Export",
  "/admin/sports": "Sports List",
  "/admin/announcements": "Announcements",
  "/admin/team": "Team Events",
  "/admin/dev-team": "Dev Team",
}

export const PageLayout: React.FC<any> = ({ children, className }) => {
  const history = useHistory();
  const location = useLocation();
  const router = useIonRouter();
  //   const logout = useStoreActions<any>((actions) => actions.logOut);

  const pathname = location.pathname;
  const logOut = () => {
    // logout();
    localStorage.removeItem('stage')
    localStorage.removeItem('Identity')
    router.push("/", "none", "replace")
    window.location.reload()
  }
  const Identity = localStorage.getItem('Identity')
  const Stage = localStorage.getItem('Stage')
  return (
    <>
      <IonHeader hidden={pathname === "/login" || pathname === "/signup"}>
        <IonToolbar>
          <IonButtons slot="start">
            <IonMenuButton autoHide={true} />
          </IonButtons>
          <IonButtons slot="primary" style={{position:'absolute', scale:'0.7', right:'-19px'}}>
            {(Identity && Stage !== '3') ? (//onclick={logout}
              <IonButton onClick={logOut} >
                <div className="Btn">
                  <div className="sign">
                    <svg viewBox="0 0 512 512">
                      <path d="M377.9 105.9L500.7 228.7c7.2 7.2 11.3 17.1 11.3 27.3s-4.1 20.1-11.3 27.3L377.9 406.1c-6.4 6.4-15 9.9-24 9.9c-18.7 0-33.9-15.2-33.9-33.9l0-62.1-128 0c-17.7 0-32-14.3-32-32l0-64c0-17.7 14.3-32 32-32l128 0 0-62.1c0-18.7 15.2-33.9 33.9-33.9c9 0 17.6 3.6 24 9.9zM160 96L96 96c-17.7 0-32 14.3-32 32l0 256c0 17.7 14.3 32 32 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-64 0c-53 0-96-43-96-96L0 128C0 75 43 32 96 32l64 0c17.7 0 32 14.3 32 32s-14.3 32-32 32z"></path>
                    </svg>
                  </div>
                  <div className="text">Logout</div>
                </div>
              </IonButton>
            ) : null}
          </IonButtons>


          <IonTitle>{TOOLBAR_TITLE[pathname]}</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent className={className}>
        {children}
      </IonContent>
    </>
  );
};
